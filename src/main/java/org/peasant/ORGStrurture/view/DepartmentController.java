package org.peasant.ORGStrurture.view;

import org.peasant.ORGStructure.model.Department;
import org.peasant.util.web.faces.JsfUtil;
import org.peasant.util.web.faces.JsfUtil.PersistAction;
import org.peasant.ORGStructure.facade.DepartmentFacade;
...
public class DepartmentController implements Serializable {
    ...
    @EJB
    protected org.peasant.ORGStructure.facade.DepartmentFacade ejbFacade;
    private List<Department> items = null;
    private Department created;
    private List<Department> selectedItems;
    ...
    public void init() {
        ...
        this.bundle = ResourceBundle.getBundle("/org/peasant/ORGStructure_i18n");
    }

    public Department getCreated() {
        return created;
    }

    public void setCreated(Department created) {
    ...
    public void create() {
        persist(PersistAction.CREATE, bundle.getString("DepartmentCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, bundle.getString("DepartmentUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, bundle.getString("DepartmentDeleted"));
        if (!JsfUtil.isValidationFailed()) {
    ...
    public List<Department> searchItems() {

        items = findItemsByConditions(construtSearchParams(this.searchCons));
        return items;
    }

    protected List<Department> findItemsByConditions(Map<String, Object> params) {
    ...
    public List<Department> allItems() {
    ...
    protected void persist(PersistAction persistAction, String successMessage) {
        ...
        try {
            ...
            switch (persistAction) {
                ...
                default: {
                    for (Department selected : selectedItems) {
                        if (selected != null) {
                            setEmbeddableKeys();
                            switch (persistAction) {
                                case DELETE:
    ...
    @FacesConverter(forClass = Department.class, value = "Department")
    public static class DepartmentFacesConverter implements Converter {
